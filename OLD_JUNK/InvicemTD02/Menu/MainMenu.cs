...
public partial class MainMenu : Node2D
{
	...
	{
		Level.GetNode("Players").GetNode<Player>(id.ToString()).QueueFree();
	}
	...
	{
		var PlayerScene = (PackedScene)ResourceLoader.Load("res://Player/player.tscn");
		var player = PlayerScene.Instantiate();
		Level.GetNode("Players").AddChild(player);
		var playerID = player.GetIndex();


		var MapScene = (PackedScene)ResourceLoader.Load("res://tile_map.tscn");
		var map = MapScene.Instantiate();
		var mapID = map.GetIndex();
		player.AddChild(map);
	}
