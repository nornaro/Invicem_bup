...
func setPlayer(playerID):
	playerdata = {
		...
		"Name"	: $CanvasLayer/VBoxContainer/Name,
		"Health": MaxHealth,
		"Spawn"	: $Players.get_node(playerID).position,
		"Area"	: $Maps.get_node(playerID),
		"Color" : Color(randf_range(0,1), randf_range(0,1), randf_range(0,1)),
		"Node"	: $Players.get_node(playerID)
	}
	Player[playerID] = playerdata
	#print($Players.get_node(playerID).position)
...
func _on_host_button_pressed():
	...
	multiplayer.peer_connected.connect($Maps.add)
	multiplayer.peer_disconnected.connect($Maps.remove)
	$Players.add(1)
